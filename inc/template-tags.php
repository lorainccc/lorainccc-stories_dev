...
if ( ! function_exists( 'lccc_stories_posted_on' ) ) :
/**
 * Prints HTML with meta information for the current post-date/time and author.
 */
function lccc_stories_posted_on() {
...
if ( ! function_exists( 'lccc_stories_entry_footer' ) ) :
/**
 * Prints HTML with meta information for the categories, tags and comments.
 */
function lccc_stories_entry_footer() {
	// Hide category and tag text for pages.
	if ( 'post' == get_post_type() ) {
		/* translators: used between list items, there is a space after the comma */
		$categories_list = get_the_category_list( esc_html__( ', ', 'lccc-framework' ) );
		if ( $categories_list && lccc_stories_categorized_blog() ) {
 ...
 * @return bool
 */
function lccc_stories_categorized_blog() {
	if ( false === ( $all_the_cool_cats = get_transient( 'lccc_stories_categories' ) ) ) {
		...
		set_transient( 'lccc_stories_categories', $all_the_cool_cats );
	}

	if ( $all_the_cool_cats > 1 ) {
		// This blog has more than 1 category so lccc_stories_categorized_blog should return true.
		return true;
	} else {
		// This blog has only 1 category so lccc_stories_categorized_blog should return false.
		return false;
	}
}

/**
 * Flush out the transients used in lccc_stories_categorized_blog.
 */
function lccc_stories_category_transient_flusher() {
	if ( defined( 'DOING_AUTOSAVE' ) && DOING_AUTOSAVE ) {
		return;
	}
	// Like, beat it. Dig?
	delete_transient( 'lccc_stories_categories' );
}
add_action( 'edit_category', 'lccc_stories_category_transient_flusher' );
add_action( 'save_post',     'lccc_stories_category_transient_flusher' );
